...
import com.tadtab.core.pojo.Cart;
...
public class ProductController
{
  ...
  public String saveProduct(@ModelAttribute("productALL") Product product, Model model)
  {
    ...
    List<Product> currentValidProducts = productDAO.returnPruductList();
  ...
  public String deletProduct(@ModelAttribute("id") int id, Model model) {
    productDAO.deleteProductFromDB(id);
    
    List<Product> cartListFromDeleteMethod = cart.getCartList();
    
    for (Product prod : cartListFromDeleteMethod) {
      if ((prod.equals(null)) && (prod.getId() == id)) {
        cartListFromDeleteMethod.remove(prod);
      }
    }
